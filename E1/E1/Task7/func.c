...
void calc_acc_looseBC(double *a, double *u, double m_o, double m_c, double kappa, int size_of_u)
{
    /* Declaration of variables */
    int i;
    
    /* Calculating the acceleration on the boundaries */
    a[0] = kappa*((u[1] - u[0]))/m_o;
    a[size_of_u - 1] = kappa*(u[size_of_u - 2] - u[size_of_u - 1] )/m_o;
    
    /* Calculating the acceleration of the inner points */
    for (i = 1; i < size_of_u - 1; i++){
        a[i] = kappa*(u[i - 1] - 2*u[i] + u[i + 1])/m_c;
    }
}
...
double calc_ke(double *v, int size_of_v, double m_o, double m_c)
{
    /* Declaration of variables */
    int i;
    double e = 0; 
