/*
E1_main.c
 
Created by AL on 2013-10-24.
Further developed by Martin Gren on 2014-10-20.
*/


#include <stdio.h>
#include <math.h>
#include <stdlib.h>
#include "func.h"
#include "fft_func.h"
#define PI 3.141592653589
#define nbr_of_timesteps 65536 /* nbr_of_timesteps+1 = power of 2, for best speed */
#define nbr_of_particles 3 /* The number of particles is 3 */


/* Main program */
int main()
{
	/* Declartion of variables */
	double timestep;
	int i,j;
	double timestep_sq,current_time;
	double kappa;
	double a_0; /* length of o-C-o bound*/
	/* mass o and c */
	double m_o;
	double m_c; 

	/* declare file variable */
	FILE *file;
	FILE *file2;

	/* displacement, velocity and acceleration */
	double q[nbr_of_particles];
	double v[nbr_of_particles];
	double a[nbr_of_particles]; 


	/* Allocating memory for large vectors */
	/* displacements for writing to file */
	double *q_1 = malloc((nbr_of_timesteps+1) * sizeof (double));
	double *q_2 = malloc((nbr_of_timesteps+1) * sizeof (double));
	double *q_3 = malloc((nbr_of_timesteps+1) * sizeof (double));
	double *E_tot = malloc((nbr_of_timesteps+1) * sizeof (double));
	double *E_pot =  malloc((nbr_of_timesteps+1) * sizeof (double));
	double *E_kin =  malloc((nbr_of_timesteps+1) * sizeof (double));
	double *P = malloc((nbr_of_timesteps+1) * sizeof (double));
	double *f = malloc((nbr_of_timesteps+1) * sizeof (double));
	/* Set variables */
	timestep = 0.001;
	kappa = 1.0;
	timestep_sq = timestep * timestep;
	
	kappa = 1.6/1.60218; /* (eV/Å^2) */ 
	m_o = 15.9999 * 1.0364 * pow(10,-4); /* (eV(ps)^2/Å^2) */
	m_c = 12.0107 * 1.0364 * pow(10,-4); /* (eV(ps)^2/Å^2) */
	a_0 = 1.1621; /* (Å) */


	
	/* Initial conditions */
	/* Set initial displacements and velocites */
	q[0] = 0.5;
	q[1] = -0.2;
	q[2] = -0.5;
	v[0] = 0;

	for (i = 1; i < nbr_of_particles; i++) {
		//q[i] = -0.1;
		v[i] = 0;
	}

	q_1[0] = q[0];
	q_2[0] = q[1];
	q_3[0] = q[2];

	/* Calculate initial accelerations based on initial displacements */
	/* calc_acc(a, q, m, kappa, nbr_of_particles); */
	calc_acc_looseBC(a, q, m_o, m_c, kappa, nbr_of_particles);

	/* timesteps according to velocity Verlet algorithm */
	for (i = 1; i < nbr_of_timesteps + 1; i++) {
		/* v(t+dt/2) */
		for (j = 0; j < nbr_of_particles; j++) {
		    v[j] += timestep * 0.5 * a[j];
		} 

		/* q(t+dt) */
		for (j = 0; j < nbr_of_particles; j++) {
		    q[j] += timestep * v[j];
		}

		/* a(t+dt) */
		/* calc_acc(a, q, m, kappa, nbr_of_particles); */
		calc_acc_looseBC(a, q, m_o, m_c, kappa, nbr_of_particles);
		
		/* v(t+dt) */
		for (j = 0; j < nbr_of_particles; j++) {
		    v[j] += timestep * 0.5 * a[j];
		} 

		/* Save the displacement of the three atoms */
		q_1[i] = q[0];
		q_2[i] = q[1];
		q_3[i] = q[2];
		E_pot[i] = calc_pe(q, kappa, nbr_of_particles);
		E_kin[i] = calc_ke(v, nbr_of_particles, m_o, m_c);
		E_tot[i] = E_kin[i] + E_pot[i];	
	}

	/* Print displacement data to output file */
	file = fopen("disp_CO2.dat","w");

	for (i = 0; i < nbr_of_timesteps + 1; i++) {
		current_time = i * timestep;
		fprintf(file, "%.4f \t %e \t %e \t %e", current_time, q_1[i], q_2[i], q_3[i] );	
		fprintf(file, "\n");
	}
	fclose(file);

	/* Print energy data to output file */
	file = fopen("energy_CO2.dat","w");

	for (i = 0; i < nbr_of_timesteps + 1; i++) {
		current_time = i * timestep;
		fprintf(file, "%.4f \t %e \t %e \t %e", current_time, E_tot[i], E_kin[i], E_pot[i] );	
		fprintf(file, "\n");
	}
	fclose(file);

	powerspectrum(q_1,P, nbr_of_timesteps);
	powerspectrum_shift(P,nbr_of_timesteps);

	fft_freq_shift(f, timestep, nbr_of_timesteps);

	//Save powerspectrum data in file 
	file2 = fopen("powerspectrum_CO2.dat","w");
	for (i = 0; i < nbr_of_timesteps; i++)	{
		fprintf (file2,"%e \t %e \n", f[i], P[i]);
	}
	
	fclose(file2);

	/* Free allocated memory */ 
	free(q_1); q_1 = NULL;
	free(q_2); q_2 = NULL;
	free(q_3); q_3 = NULL;
	free(E_pot); E_pot = NULL;
	free(E_tot); E_tot = NULL;
	free(E_kin); E_kin = NULL;
	free(P); P = NULL;
	free(f); f = NULL;
	return 0;    
}
